...
class TestCatAccessor:
    ...
    def test_set_categories_setitem(self):
        ...
        df = DataFrame({"Survived": [1, 0, 1], "Gender": [0, 1, 1]}, dtype="category")
        ...
        assert list(df["Gender"]) == ["female", "male", "male"]
        assert list(df["Survived"]) == ["Yes", "No", "Yes"]

        df["Gender"] = Categorical(df["Gender"], categories=["female", "male"], ordered=False)
